...
def gerar_imagem_free(prompt: str, destino: Path):
...
def gerar_imagem_openai(prompt: str, destino: Path):
    resp = requests.post(
...
def gerar_imagem_ideogram(prompt: str, destino: Path):
    resp = requests.post(
...
def main():
    ...
    for i, cena in enumerate(roteiro["cenas"], start=1):
        ...
        gerar_imagem(cena["descricao_visual"], destino)
